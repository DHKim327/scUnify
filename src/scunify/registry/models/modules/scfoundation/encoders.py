...
import torch
# Original RelatedWorks source had ``import torchvision`` + ``from load import *``
# (with ``sys.path.append("../model/")``). torchvision is unused; ``load.py``
# only contributes ``convertconfig`` which we inline below.
...
def convertconfig(ckpt):
    """Vendored byte-level from
    ``RelatedWorks/Foundations/scFoundation/model/load.py:76``. Reshapes a
    raw scFoundation pretrained checkpoint (with nested ``config``/``model_config``)
    into the flat dict that :class:`MAEAutobinencoder` consumes.
    """
    newconfig = {}
    newconfig['config'] = {}
