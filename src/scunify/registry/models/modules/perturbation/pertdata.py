    ...
    def get_dataloader(self, batch_size, test_batch_size = None):
        ...
        if self.split == 'no_split':
            ...
            for i in splits:
                cell_graphs[i] = []
                for p in self.set2conditions[i]:
                    # PertData filters perturbations not in GO graph from
                    # ``self.adata`` (and thus from ``self.dataset_processed``)
                    # but the persisted ``set2conditions`` split.pkl still lists
                    # them. Use .get() so stale split entries are silently
                    # skipped — paper-faithful intent (GO filter) is preserved.
                    graphs_p = self.dataset_processed.get(p)
                    if graphs_p is not None:
                        cell_graphs[i].extend(graphs_p)
