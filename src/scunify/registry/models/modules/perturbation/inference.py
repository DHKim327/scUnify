...
# ---------------------------------------------------------------------------- #
# compute_perturbation_metrics_with_de_idx — paper-faithful 4-metric calc
# using cache-resolved de_idx (avoids ENSG↔symbol KeyError that happens
# when var.index is gene symbols but uns['rank_genes_groups_cov_all']
# stores Ensembl IDs).
# ---------------------------------------------------------------------------- #
def build_cond2de_idx(dataset_processed):
    """Build ``{condition: np.ndarray}`` from ``PertData.dataset_processed``
    (= condition → list[Data]). Reads ``Data.de_idx`` (already-resolved
    var-axis indices, sentinel ``-1`` filtered)."""
    out = {}
    for cond, graphs in dataset_processed.items():
        if cond == "ctrl" or not graphs:
            continue
        de = list(graphs[0].de_idx)
        de_arr = np.asarray([int(x) for x in de])
        de_arr = de_arr[de_arr >= 0]
        if len(de_arr) > 0:
            out[cond] = de_arr
    return out


def compute_perturbation_metrics_with_de_idx(
    results: Dict,
    ctrl_adata: AnnData,
    cond2de_idx: Dict,
) -> Dict:
    """Returns: pearson, pearson_de, pearson_delta, pearson_de_delta (paper-faithful)."""
    pert_cat = results["pert_cat"]
    pred = results["pred"]
    truth = results["truth"]
    mean_ctrl = np.array(ctrl_adata.X.mean(0)).flatten()
    out = {"pearson": [], "pearson_de": [], "pearson_delta": [], "pearson_de_delta": []}
    conds = np.unique(pert_cat)
    cond2idx = {c: np.where(pert_cat == c)[0] for c in conds}

    for c in conds:
        if c == "ctrl":
            continue
        idx = cond2idx[c]
        true_m = truth[idx].mean(0)
        pred_m = pred[idx].mean(0)
        true_d = true_m - mean_ctrl
        pred_d = pred_m - mean_ctrl

        if true_m.std() > 0 and pred_m.std() > 0:
            out["pearson"].append(pearsonr(pred_m, true_m)[0])
        if true_d.std() > 0 and pred_d.std() > 0:
            out["pearson_delta"].append(pearsonr(pred_d, true_d)[0])

        de_idx = cond2de_idx.get(c)
        if de_idx is None or len(de_idx) < 5:
            continue
        true_de = true_m[de_idx]
        pred_de = pred_m[de_idx]
        true_de_d = true_de - mean_ctrl[de_idx]
        pred_de_d = pred_de - mean_ctrl[de_idx]
        if true_de.std() > 0 and pred_de.std() > 0:
            out["pearson_de"].append(pearsonr(pred_de, true_de)[0])
        if true_de_d.std() > 0 and pred_de_d.std() > 0:
            out["pearson_de_delta"].append(pearsonr(pred_de_d, true_de_d)[0])

    return {k: float(np.mean(v)) if v else float("nan") for k, v in out.items()}


# ---------------------------------------------------------------------------- #
# compute_perturbation_metrics — vendored byte-level from
# RelatedWorks/Foundations/scGPT/scgpt/utils/util.py:429-595
# Computes condition-mean Pearson / Pearson_delta over all genes and top-20 DE
# genes. Used by both ScGPTPerturbationMixin and ScFoundationPerturbationMixin
# as the canonical paper metric (replaces gears.inference.compute_metrics for
# the main reporting metric set).
# ---------------------------------------------------------------------------- #
def compute_perturbation_metrics(
    ...
    Args:
        results: dict with keys ``pert_cat`` (n_cells,), ``pred`` (n_cells,
            n_genes), ``truth`` (n_cells, n_genes).
        ctrl_adata: AnnData of the control condition (must contain
            ``uns['rank_genes_groups_cov_all']`` and optionally
            ``uns['top_non_dropout_de_20']``).
        non_zero_genes: if True, only consider non-zero genes in ground truth
            for the all-gene Pearson and use ``top_non_dropout_de_20`` for DE.
        return_raw: if True, return per-condition lists; else mean.

    Returns:
        dict: ``pearson``, ``pearson_de``, ``pearson_delta``,
              ``pearson_de_delta``.
    """
