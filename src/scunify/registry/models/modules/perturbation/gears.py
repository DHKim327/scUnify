...
class GEARS:
    def __init__(self, pert_data, 
        ...
        # Newer scipy.sparse rejects pandas Series masks (calls Series.nonzero
        # which is removed in pandas 2.x); convert to numpy bool array first.
        _ctrl_mask = (self.adata.obs.condition == 'ctrl').values
        self.ctrl_expression = torch.tensor(np.mean(self.adata.X[_ctrl_mask], axis = 0)).reshape(-1,).to(self.device)
